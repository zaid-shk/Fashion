...
const BestSeller = () => {
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-5 items-center justify-center py-8">
        {Products.map((item, index) => {
          return (
            <div
              key={index}
              className="flex flex-col border border-[#1f1f1f1f] rounded-3xl p-2"
            >
              <Card image={item.image} height="450px" width="300px" />
              <div className="flex flex-col px-5 py-3">
                <p className="text-lg tracking-wide text-zinc-700/80">
                  {item.title}
                </p>
                <h4 className="text-2xl font-bold text-neutral-900 tracking-widest">
                  {item.price}
                </h4>
              </div>
            </div>
